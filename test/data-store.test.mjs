...
test('memory repository handles an end-to-end job lifecycle', async () => {
  ...
  const updatedContact = await store.updateContact(
    contact.id,
    {
  ...
  assert.equal(
    updatedContact.freeVideoUsed,
    true,
  );

  assert.equal(
    (await store.getContactById(contact.id))
      .freeVideoUsed,
    true,
  );
  ...
  assert.equal(
    await store.hasActiveJob(contact.id),
    true,
  );
  ...
  assert.equal(
    (await store.getLatestJob(contact.id)).status,
    'completed',
  );
});
