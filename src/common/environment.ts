import { AiProviderName, AiRoutingProviderName } from '../ai/ai.types';
...
const aiProviderOrder = (value: unknown): AiProviderName[] => {
  ...
  const result = (optional(value) ?? 'gemini,openai,claude')
    .split(',')
  ...
  return result.length ? [...new Set(result)] : allowed;
};
...
export function validateEnvironment(
  ...
  const videoProvider = (
    optional(raw.VIDEO_PROVIDER) ?? 'mock'
  ) as VideoProviderName;

  const legacyAi = optional(raw.AI_TEXT_PROVIDER);
  ...
  if (!['mock', 'runway'].includes(videoProvider)) {
    throw new Error('VIDEO_PROVIDER must be mock or runway');
  }

  if (
    !['auto', 'gemini', 'openai', 'claude'].includes(
      aiDefaultProvider,
    )
  ) {
    throw new Error(
  ...
  if (runwayDuration < 2 || runwayDuration > 10) {
    throw new Error(
  ...
  return {
    ...
    PORT: positiveInteger(raw.PORT, 3000, 'PORT'),

    META_GRAPH_API_VERSION:
      optional(raw.META_GRAPH_API_VERSION) ?? 'v25.0',

    META_ACCESS_TOKEN: optional(raw.META_ACCESS_TOKEN),
    ...
    AI_REQUEST_TIMEOUT_MS: positiveInteger(
      raw.AI_REQUEST_TIMEOUT_MS,
      60_000,
      'AI_REQUEST_TIMEOUT_MS',
    ),

    AI_RETRY_ATTEMPTS: positiveInteger(
      raw.AI_RETRY_ATTEMPTS,
      3,
      'AI_RETRY_ATTEMPTS',
    ),

    AI_MAX_REPLY_CHARS: positiveInteger(
    ...
    GEMINI_MODEL:
      optional(raw.GEMINI_MODEL) ?? 'gemini-3.8-flash',

    GEMINI_FALLBACK_MODEL:
      optional(raw.GEMINI_FALLBACK_MODEL) ??
      'gemini-3.5-flash-lite',

    OPENAI_API_KEY: optional(raw.OPENAI_API_KEY),

    OPENAI_MODEL:
      optional(raw.OPENAI_MODEL) ?? 'gpt-5.6-terra',
    ...
    OPENAI_TTS_VOICE:
      optional(raw.OPENAI_TTS_VOICE) ?? 'marin',
    ...
    CLAUDE_MODEL:
      optional(raw.CLAUDE_MODEL) ?? 'claude-sonnet-5',
    ...
    RUNWAYML_API_SECRET: optional(
      raw.RUNWAYML_API_SECRET,
    ),

    RUNWAY_MODEL:
      optional(raw.RUNWAY_MODEL) ?? 'gen4.5',

    RUNWAY_DURATION: runwayDuration,

    RUNWAY_RATIO:
      optional(raw.RUNWAY_RATIO) ?? '720:1280',

    SUPABASE_URL: optional(raw.SUPABASE_URL),
    ...
    SUPABASE_BUCKET:
      optional(raw.SUPABASE_BUCKET) ?? 'adyarat-media',

    WEBHOOK_POLL_MS: positiveInteger(
      raw.WEBHOOK_POLL_MS,
      1000,
      'WEBHOOK_POLL_MS',
    ),

    GENERATION_POLL_MS: positiveInteger(
      raw.GENERATION_POLL_MS,
      3000,
      'GENERATION_POLL_MS',
    ),

    GENERATION_STALE_MS: positiveInteger(
      raw.GENERATION_STALE_MS,
      20 * 60 * 1000,
      'GENERATION_STALE_MS',
    ),

    MAX_IMAGE_BYTES: positiveInteger(
      raw.MAX_IMAGE_BYTES,
      5 * 1024 * 1024,
      'MAX_IMAGE_BYTES',
    ),

    MAX_AUDIO_BYTES: positiveInteger(
      raw.MAX_AUDIO_BYTES,
      25 * 1024 * 1024,
      'MAX_AUDIO_BYTES',
    ),

    MAX_DOCUMENT_BYTES: positiveInteger(
      raw.MAX_DOCUMENT_BYTES,
      20 * 1024 * 1024,
      'MAX_DOCUMENT_BYTES',
    ),

    MAX_DOCUMENT_CHARS: positiveInteger(
      raw.MAX_DOCUMENT_CHARS,
      500_000,
      'MAX_DOCUMENT_CHARS',
    ),

    DOCUMENT_CHUNK_CHARS: positiveInteger(
