...
type ContactPatch = Partial<
  Pick<Contact, 'profileName' | 'state' | 'freeVideoUsed' | 'pendingImagePath' | 'pendingImageMime'>
>;
...
export class DataStoreService {
  ...
  async getContactByWaId(waId: string): Promise<Contact | undefined> {
    if (!this.supabase.isEnabled()) {
      const id = this.contactsByWaId.get(waId);
  ...
  async updateContact(id: string, patch: ContactPatch): Promise<Contact> {
    ...
    if ('freeVideoUsed' in patch) {
      dbPatch.free_video_used = patch.freeVideoUsed ?? false;
    }

    if ('pendingImagePath' in patch) {
      dbPatch.pending_image_path = patch.pendingImagePath ?? null;
    }

    if ('pendingImageMime' in patch) {
      dbPatch.pending_image_mime = patch.pendingImageMime ?? null;
    }
  ...
  async getStaleProcessingJobs(
    ...
    return (data ?? []).map((row) => this.toGenerationJob(row));
  }
  ...
  private toContact(row: DbContact): Contact {
    return {
      ...
      state: row.state,
      freeVideoUsed: row.free_video_used,
      pendingImagePath:
