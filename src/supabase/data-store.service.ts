...
type ContactPatch = Partial<
  Pick<Contact, 'profileName' | 'state' | 'pendingImagePath' | 'pendingImageMime'>
>;

type JobPatch = Partial<
  Pick<GenerationJob, 'status' | 'outputStoragePath' | 'providerJobId' | 'errorMessage'>
>;
...
export class DataStoreService {
  ...
  async enqueueWebhook(eventKey: string, payload: unknown): Promise<boolean> {
    if (!this.supabase.isEnabled()) {
      if (this.eventIdsByKey.has(eventKey)) return false;
    ...
    const { error } = await this.supabase.client.from('webhook_events').insert({
      event_key: eventKey,
      payload,
      status: 'pending',
    });
  ...
  async claimNextWebhook(): Promise<WebhookEvent | undefined> {
    ...
    const { data: candidate, error: readError } = await this.supabase.client
      .from('webhook_events')
      .select('*')
      .eq('status', 'pending')
  ...
  async failWebhook(id: string, errorMessage: string): Promise<void> {
  ...
  async getOrCreateContact(waId: string, profileName?: string): Promise<Contact> {
    if (!this.supabase.isEnabled()) {
    ...
    if (existing) {
      if (profileName && profileName !== existing.profile_name) {
        return this.updateContact(existing.id, { profileName });
      }
    ...
    const { data, error } = await this.supabase.client
      .from('wa_contacts')
      .insert({ wa_id: waId, profile_name: profileName ?? null, state: 'new' })
      .select('*')
      .single<DbContact>();
    if (error) throw error;
  ...
  async updateContact(id: string, patch: ContactPatch): Promise<Contact> {
    ...
    if ('profileName' in patch) dbPatch.profile_name = patch.profileName ?? null;
    if ('state' in patch) dbPatch.state = patch.state;
    if ('pendingImagePath' in patch) dbPatch.pending_image_path = patch.pendingImagePath ?? null;
    if ('pendingImageMime' in patch) dbPatch.pending_image_mime = patch.pendingImageMime ?? null;
  ...
  async recordMessage(message: MessageRecord): Promise<boolean> {
    if (!this.supabase.isEnabled()) {
      if (this.messageIds.has(message.waMessageId)) return false;
      this.messageIds.add(message.waMessageId);
      this.messageStatuses.set(message.waMessageId, message.status);
      return true;
    }

    const { error } = await this.supabase.client.from('wa_messages').insert({
  ...
  async hasActiveJob(contactId: string): Promise<boolean> {
    if (!this.supabase.isEnabled()) {
      return [...this.jobs.values()].some(
        (job) => job.contactId === contactId && ['queued', 'processing'].includes(job.status),
      );
    }
  ...
  async claimNextJob(): Promise<GenerationJob | undefined> {
    ...
    const { data, error } = await this.supabase.client
      ...
      .maybeSingle<DbGenerationJob>();
    if (error) throw error;
    return data ? this.toGenerationJob(data) : undefined;
  }

  async updateJob(id: string, patch: JobPatch): Promise<GenerationJob> {
    if (!this.supabase.isEnabled()) {
    ...
    if ('outputStoragePath' in patch) dbPatch.output_storage_path = patch.outputStoragePath ?? null;
    if ('providerJobId' in patch) dbPatch.provider_job_id = patch.providerJobId ?? null;
    if ('errorMessage' in patch) dbPatch.error_message = patch.errorMessage ?? null;
  ...
  async getLatestJob(contactId: string): Promise<GenerationJob | undefined> {
    if (!this.supabase.isEnabled()) {
      return [...this.jobs.values()]
        .filter((job) => job.contactId === contactId)
  ...
  private toContact(row: DbContact): Contact {
    return {
      ...
      state: row.state,
      pendingImagePath: row.pending_image_path ?? undefined,
      pendingImageMime: row.pending_image_mime ?? undefined,
      createdAt: row.created_at,
      updatedAt: row.updated_at,
    };
  }

  private toWebhookEvent(row: DbWebhookEvent): WebhookEvent {
    return {
      id: row.id,
  ...
  private toGenerationJob(row: DbGenerationJob): GenerationJob {
    return {
      id: row.id,
